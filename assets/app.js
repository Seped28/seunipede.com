...
async function loadRepos() {
  ...
  try {
    ...
    // 1. Try to load from Cache first to bypass Rate Limit
    if (cachedData && cachedTimestamp && (Date.now() - cachedTimestamp < ONE_HOUR)) {
      ...
      // 2. Fetch from GitHub only if cache is old or missing
      repos = await ghFetch(
    ...
    if (typeof buildSkills === 'function') buildSkills(langs);
    ...
    displayed.forEach((repo, idx) => {
      ...
      const color   = typeof langColor === 'function' ? langColor(repo.language) : '#ccc';
      ...
      card.innerHTML = `
        ...
        <div class="project-desc">${typeof escHtml === 'function' ? escHtml(desc) : desc}</div>
        <div class="project-meta">
          <div class="meta-item">⭐ ${repo.stargazers_count}</div>
          <div class="meta-item">🍴 ${repo.forks_count}</div>
          <div class="meta-item">📅 ${updated}</div>
          ${topic ? `<div class="project-topic">${typeof escHtml === 'function' ? escHtml(topic) : topic}</div>` : ''}
        </div>
      `;

      grid.appendChild(card);
      if (window.revealObs) revealObs.observe(card);
    });

  } catch (err) {
    console.warn('Repos fetch failed:', err.message);
    
    // FINAL FALLBACK: If API fails, try to show the old cache even if it's expired
    const backup = localStorage.getItem(CACHE_KEY);
    if (backup) {
       console.log("Rate limit hit, showing expired cache as fallback.");
       // Re-run the function but force it to use backup data here
       // Or simply show the error if no backup exists
    }
